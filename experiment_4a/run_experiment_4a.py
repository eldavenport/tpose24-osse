#!/usr/bin/env python
"""
run_experiment_4.py — sample the model once, estimate w for every experiment_1
configuration/cell, and save arrays plus a master metrics table.

Same as experiment_2 (w_est extrapolates the velocity shear to the surface and
integrates w from w=0 at 0 m, the compute_w_planefit default) but over a deeper
8–120 m sampling range instead of 8–70 m.
...
already exists; delete experiment_4/data/ to recompute.

Run (from the tpose env):  python experiment_4/run_experiment_4.py
"""
import glob
import json
import os
...
def main():
    ...
    if pending:
        ...
        for cfg, center_lat, pos, nc_path in pending:
            ...
            # experiment_4: extrapolate U,V to the surface and integrate w from w=0
