...
=True, w=0 at 0 m); they differ only in MAX_DEPTH (80 vs 120 m). compute_w_planefit
integrates w downward from the surface, so w_est over 8-80 m does not depend on how
deep the profile was sampled — the 8-80 m slice of a 4 profile is bit-identical to a
...
truncated to depth >= -MAX_DEPTH (80 m), into experiment_2/data/.
...
metrics.csv (stamped MIN_DEPTH=8, MAX_DEPTH=80) without opening the model.
...
MAX_DEPTH = 80   # keep in sync with run_experiment_2.py
...
def main():
    ...
    for src in src_files:
        ...
        with xr.open_dataset(src) as ds:
            ...
            # >= -MAX_DEPTH, i.e. the -80 .. 0 slice (41 interfaces), matching a
            # native 8-80 m experiment_2 file.
