...
as a stationary mooring array: they sit at fixed positions and a plane fit to
...
smaller circle is orbited faster. The choice to circle clockwise was arbitrary.
...
| choice | value |
|---|---|
| center | 0°N, 140°W (the TAO mooring, fixed) |
| gliders per array `N` | 3, 4, 5, 6 |
| circle diameters | 0.3, 0.5, 0.75, 1.0°|
...
The geometry is generated directly from `circle_common.glider_positions`.

### Truth = the fixed disk
The estimate is compared against the fixed circular disk of that diameter centered at
0°N,140°W — the area-mean over every model grid point inside the disk. The center is on
the equator. The disk truth is the same target for every `N` at a given diameter, so skill
...
cloud come from a tracer-centered region over the same bbox (`region_bbox` → `disk_select`).
...
* `circle_plot.py` — shared style + cache loaders (N colored light→dark, diameter
  light→dark, array solid / disk truth dotted).
* `make_figs.py` — w-skill and heat-flux-skill figures → `figs/`.

The distribution figures (`fit_distributions`) live at the repo
top level in `circling_gliders/` (`run_distributions.py`).
...
* `figs/`:
  ...
  * `heat_area_components`(`_flux`) — structural decomposition (true area mean vs estimated
    mean, estimate error, sub-array covariance), columns = diameter, lines colored by `N`.
  * `heat_area_reducible_d1` — reducible plane-fit / sampling errors (`w`, ∂T/∂z, heating, heat
    flux) at the 1° diameter, true vs estimate + estimate error, lines colored by `N`.
